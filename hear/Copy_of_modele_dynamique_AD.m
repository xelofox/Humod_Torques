...
%vitesse
t = num2cell(transpose(q(:,k)));
 [dpBJX dpBJY dpBJZ drBJX drBJY drBJZ drLNJX drLNJY drLNJZ drSJX_L drSJY_L drSJZ_L drSJX_R drSJY_R drSJZ_R drEJZ_L drEJZ_R drULJX drULJY drULJZ drLLJX drLLJZ drHJX_L drHJY_L drHJZ_L drHJX_R drHJY_R drHJZ_R drKJZ_L drKJZ_R drAJX_L drAJY_L drAJZ_L drAJX_R drAJY_R drAJZ_R]=deal(t{:});
 [dpBJX dpBJY dpBJZ drBJX drBJY drBJZ drLNJX drLNJY drLNJZ drSJX_L drSJY_L drSJZ_L drSJX_R drSJY_R drSJZ_R drEJZ_L drEJZ_R drULJX drULJY drULJZ drLLJX drLLJZ drHJX_L drHJY_L drHJZ_L drHJX_R drHJY_R drHJZ_R drKJZ_L drKJZ_R drAJX_L drAJY_L drAJZ_L drAJX_R drAJY_R drAJZ_R]=ainit(dpBJX, dpBJY, dpBJZ, drBJX, drBJY, drBJZ, drLNJX, drLNJY, drLNJZ, drSJX_L, drSJY_L, drSJZ_L, drSJX_R, drSJY_R, drSJZ_R, drEJZ_L, drEJZ_R, drULJX, drULJY, drULJZ, drLLJX, drLLJZ, drHJX_L, drHJY_L, drHJZ_L, drHJX_R, drHJY_R, drHJZ_R, drKJZ_L, drKJZ_R, drAJX_L, drAJY_L, drAJZ_L, drAJX_R, drAJY_R, drAJZ_R,2);

v_pelvis=aeval(ajac(c_pelvis))*dq;
v_abdomen=aeval(ajac(c_abdomen))*dq;
v_thorax=aeval(ajac(c_thorax))*dq;
v_head=aeval(ajac(c_head))*dq;
v_upperArmL=aeval(ajac(c_upperArmL))*dq;
v_lowerArmL=aeval(ajac(c_lowerArmL))*dq;
v_upperArmR=aeval(ajac(c_upperArmR))*dq;
v_lowerArmR=aeval(ajac(c_lowerArmR))*dq;
v_thighL=aeval(ajac(c_thighL))*dq;
v_shankL=aeval(ajac(c_shankL))*dq;
v_footL=aeval(ajac(c_footL))*dq;
v_thighR=aeval(ajac(c_thighR))*dq;
v_shankR=aeval(ajac(c_shankR))*dq;
v_footR=aeval(ajac(c_footR))*dq;


clear c_pelvis c_abdomen c_thorax c_head c_upperArmL c_lowerArmL c_upperArmR c_lowerArmR c_thighL c_shankL c_footL c_thighR c_shankR c_footR;
clear pBJX pBJY pBJZ rBJX rBJY rBJZ rLNJX rLNJY rLNJZ rSJX_L rSJY_L rSJZ_L rSJX_R rSJY_R rSJZ_R rEJZ_L rEJZ_R rULJX rULJY rULJZ rLLJX rLLJZ rHJX_L rHJY_L rHJZ_L rHJX_R rHJY_R rHJZ_R rKJZ_L rKJZ_R rAJX_L rAJY_L rAJZ_L rAJX_R rAJY_R rAJZ_R;
clear abdomen age bodyHeight bodyMass equipmentMass foot_L foot_R gender head joints lowerArm_L lowerArm_R origin pelvis shank_L shank_R subject thigh_L thigh_R thorax upperArm_L upperArm_R;

%% Energie cin�tique
'calcul de T'
T_pelvis=0.5*m_pelvis*conj(v_pelvis')*v_pelvis+0.5*[drBJX drBJY drBJZ]*(I_pelvis*[drBJX; drBJY; drBJZ]);
%T_pelvis=simplify(T_pelvis);
T_abdomen=0.5*m_abdomen*conj(v_abdomen')*v_abdomen+0.5*[drLLJX 0 drLLJZ]*(I_abdomen*[drLLJX; 0; drLLJZ]);
%T_abdomen=simplify(T_abdomen);
T_thorax=0.5*m_thorax*conj(v_thorax')*v_thorax+0.5*[drULJX drULJY drULJZ]*(I_thorax*[drULJX; drULJY; drULJZ]);
%T_thorax=simplify(T_thorax);
T_head=0.5*m_head*conj(v_head')*v_head+0.5*[drLNJX drLNJZ drLNJZ]*(I_head*[drLNJX; drLNJZ; drLNJZ]);
%T_head=simplify(T_head);
T_upperArmL=0.5*m_upperArmL*conj(v_upperArmL')*v_upperArmL+0.5*[drSJX_L drSJY_L drSJZ_L]*(I_upperArmL*[drSJX_L; drSJY_L; drSJZ_L]);
%T_upperArmL=simplify(T_upperArmL);
T_lowerArmL=0.5*m_lowerArmL*conj(v_lowerArmL')*v_lowerArmL+0.5*[0 0 drEJZ_L]*(I_lowerArmL*[0; 0; drEJZ_L]);
%T_lowerArmL=simplify(T_lowerArmL);
T_upperArmR=0.5*m_upperArmR*conj(v_upperArmR')*v_upperArmR+0.5*[drSJX_R drSJY_R drSJZ_R]*(I_upperArmR*[drSJX_R; drSJY_R; drSJZ_R]);
%T_upperArmR=simplify(T_upperArmR);
T_lowerArmR=0.5*m_lowerArmR*conj(v_lowerArmR')*v_lowerArmR+0.5*[0 0 drEJZ_R]*(I_lowerArmR*[0; 0; drEJZ_R]);
%T_lowerArmR=simplify(T_lowerArmR);
T_thighL=0.5*m_thighL*conj(v_thighL')*v_thighL+0.5*[drHJX_L drHJY_L drHJZ_L]*(I_thighL*[drHJX_L; drHJY_L; drHJZ_L]);
%T_thighL=simplify(T_thighL);
T_shankL=0.5*m_shankL*conj(v_shankL')*v_shankL+0.5*[0 0 drKJZ_L]*(I_shankL*[0; 0; drKJZ_L]);
%T_shankL=simplify(T_shankL);
T_footL=0.5*m_footL*conj(v_footL')*v_footL+0.5*[drAJX_L drAJY_L drAJZ_L]*(I_footL*[drAJX_L; drAJY_L; drAJZ_L]);
%T_footL=simplify(T_footL);
T_thighR=0.5*m_thighR*conj(v_thighR')*v_thighR+0.5*[drHJX_R drHJY_R drHJZ_R]*(I_thighR*[drHJX_R; drHJY_R; drHJZ_R]);
%T_thighR=simpRify(T_thighR);
T_shankR=0.5*m_shankR*conj(v_shankR')*v_shankR+0.5*[0 0 drKJZ_R]*(I_shankR*[0; 0; drKJZ_R]);
%T_shankR=simpRify(T_shankR);
T_footR=0.5*m_footR*conj(v_footR')*v_footR+0.5*[drAJX_R drAJY_R drAJZ_R]*(I_footR*[drAJX_R; drAJY_R; drAJZ_R]);
