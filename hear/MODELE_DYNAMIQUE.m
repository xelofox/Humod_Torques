...
'calcul des vitesses'
%vitesse
v_pelvis=jacobian(c_pelvis,q)*dq;
v_abdomen=jacobian(c_abdomen,q)*dq;
v_thorax=jacobian(c_thorax,q)*dq;
v_head=jacobian(c_head,q)*dq;
v_upperArmL=jacobian(c_upperArmL,q)*dq;
v_lowerArmL=jacobian(c_lowerArmL,q)*dq;
v_upperArmR=jacobian(c_upperArmR,q)*dq;
v_lowerArmR=jacobian(c_lowerArmR,q)*dq;
v_thighL=jacobian(c_thighL,q)*dq;
v_shankL=jacobian(c_shankL,q)*dq;
v_footL=jacobian(c_footL,q)*dq;
v_thighR=jacobian(c_thighR,q)*dq;
v_shankR=jacobian(c_shankR,q)*dq;
v_footR=jacobian(c_footR,q)*dq;
