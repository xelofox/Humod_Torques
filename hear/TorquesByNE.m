...
%         pos_reactL=[force.copX_L(2*k);force.copY_L(2*k);force.copZ_L(2*k)]*1e-3;
%     end
    
    if (isnan(force.grfX(2*k))||isnan(force.grfY_L(2*k))||isnan(force.grfZ(2*k))||isnan(copX_L(2*k))||isnan(copY_L(2*k))||isnan(copZ_L(2*k)))
         freactL=zeros(3,1);pos_reactL=ones(3,1); 
    else
        freactL=[force.grfX(2*k)/2;force.grfY_R(2*k);force.grfZ(2*k)/2];
        pos_reactL=[copX_R(2*k);copY_R(2*k);copZ_R(2*k)]*1e-3;
    end
...
%         pos_reactR=[force.copX_R(2*k);force.copY_R(2*k);force.copZ_R(2*k)]*1e-3;
%     end

    if (isnan(force.grfX(2*k))||isnan(force.grfY_R(2*k))||isnan(force.grfZ(2*k))||isnan(copX_R(2*k))||isnan(copY_R(2*k))||isnan(copZ_R(2*k)))
         freactR=zeros(3,1);pos_reactR=ones(3,1); 
    else
        freactR=[force.grfX(2*k)/2;force.grfY_R(2*k);force.grfZ(2*k)/2];
        pos_reactR=[copX_R(2*k);copY_R(2*k);copZ_R(2*k)]*1e-3;
    end
...
%% Récupération des couples
    TauL(k,:)=transpose([cross(pos_KJL,u_shankL_z); u_shankL_z])*fi_shankL_z;
    TauR(k,:)=transpose([cross(pos_KJR,u_shankR_z); u_shankR_z])*fi_shankR_z;

    
    
    
end
clear -regexp ^I ^f ^d ^p ^k ^c ^u ^R ^g ^r ^m ^q ^o ^b ^a ^t ^s ^h ^e ^j ^l