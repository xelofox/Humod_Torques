...
freactL = zeros(3, motion.frames);
freactR = zeros(3, motion.frames);
...
freactL(1:3,singleSupport_L) = Freact(1:3,singleSupport_L);
...
freactR(1:3,singleSupport_R) = Freact(1:3,singleSupport_R);
            ...
            if any(doubleSupport)
                ...
                for doubleSupportIndex = 1:length(doubleSupportStart)
                    ...
                    if events.eventStart_L(eventIndex_L) < events.eventStart_R(eventIndex_R)
                        FX1 = Freact(1,startIndex_L:doubleSupportStart(doubleSupportIndex));
                        FX3 = Freact(1,doubleSupportEnd(doubleSupportIndex):endIndex_R);
                        FY1 = Freact(2,startIndex_L:doubleSupportStart(doubleSupportIndex));
                        FY3 = Freact(2,doubleSupportEnd(doubleSupportIndex):endIndex_R);
                        FZ1 = Freact(3,startIndex_L:doubleSupportStart(doubleSupportIndex));
                        FZ3 = Freact(3,doubleSupportEnd(doubleSupportIndex):endIndex_R);
                    else
                        FX1 = Freact(1,startIndex_R:doubleSupportStart(doubleSupportIndex));
                        FX3 = Freact(1,doubleSupportEnd(doubleSupportIndex):endIndex_L);
                        FY1 = Freact(2,startIndex_R:doubleSupportStart(doubleSupportIndex));
                        FY3 = Freact(2,doubleSupportEnd(doubleSupportIndex):endIndex_L);
                        FZ1 = Freact(3,startIndex_R:doubleSupportStart(doubleSupportIndex));
                        FZ3 = Freact(3,doubleSupportEnd(doubleSupportIndex):endIndex_L);
                    end
                    ...
                    FX_slope = abs(Freact(1,doubleSupportStart(doubleSupportIndex))) / bodyMass;
                    FX_max = max(max(abs(FX1)), max(abs(FX3))) / bodyMass;
                    FY_i = abs(FY1(end)) / bodyMass;
                    FY_slope = abs(Freact(2,doubleSupportStart(doubleSupportIndex))) / bodyMass;
                    FY_max = max(max(abs(FY1)), max(abs(FY3))) / bodyMass;
                    FZ_slope = abs(Freact(3,doubleSupportStart(doubleSupportIndex))) / bodyMass;
                    ...
                    FX22 = Freact(1,doubleSupportStart(doubleSupportIndex):doubleSupportEnd(doubleSupportIndex)) - FX21;
%                     if events.grfCorrection_L(eventIndex_L) || events.grfCorrection_R(eventIndex_R)
                        ...
                        FY22 = Freact(2,doubleSupportStart(doubleSupportIndex):doubleSupportEnd(doubleSupportIndex)) - FY21;
%                     else
%                         if events.eventStart_L(eventIndex_L) < events.eventStart_R(eventIndex_R)
...
%                     end
                    ...
                    FZ22 = Freact(3,doubleSupportStart(doubleSupportIndex):doubleSupportEnd(doubleSupportIndex)) - FZ21;
                    
                    % Combine measured during single support phase and estimated forces during double support phase
                    if events.eventStart_L(eventIndex_L) < events.eventStart_R(eventIndex_R)
                        freactL(1:3,doubleSupportStart(doubleSupportIndex):doubleSupportEnd(doubleSupportIndex)) = [FX21;  FY21; FZ21];
                        freactR(1:3,doubleSupportStart(doubleSupportIndex):doubleSupportEnd(doubleSupportIndex)) = [FX22; FY22; FZ22];

                    else
                        freactR(1:3,doubleSupportStart(doubleSupportIndex):doubleSupportEnd(doubleSupportIndex)) = [FX21;  FY21; FZ21];
                        freactL(1:3,doubleSupportStart(doubleSupportIndex):doubleSupportEnd(doubleSupportIndex)) = [FX22; FY22; FZ22];
                    end
                end
            end