...
%% from nb to 1 : effort
    ...
    fi_footL_z=facc_footL_z-[m_footL*g;cross(c_footL,m_footL*g)];
    ...
    fi_footR_z=facc_footR_z-[m_footR*g;cross(c_footR,m_footR*g)];
...
%% Torques
    ...
    HJZ_L(k)=transpose([cross(pos_HJL,u_thighL_z); u_thighL_z])*fi_thighL_z;
    HJZ_R(k)=transpose([cross(pos_HJR,u_thighR_z); u_thighR_z])*fi_thighR_z;
    HJY_L(k)=transpose([cross(pos_HJL,u_thighL_y); u_thighL_y])*fi_thighL_y;
    HJY_R(k)=transpose([cross(pos_HJR,u_thighR_y); u_thighR_y])*fi_thighR_y;
    HJX_L(k)=transpose([cross(pos_HJL,u_thighL_x); u_thighL_x])*fi_thighL_x;
    HJX_R(k)=transpose([cross(pos_HJR,u_thighR_x); u_thighR_x])*fi_thighR_x;
    
    AJZ_L(k)=transpose([cross(pos_AJL,u_footL_z); u_footL_z])*fi_footL_z;
    AJZ_R(k)=transpose([cross(pos_AJR,u_footR_z); u_footR_z])*fi_footR_z;
    AJY_L(k)=transpose([cross(pos_AJL,u_footL_y); u_footL_y])*fi_footL_y;
    AJY_R(k)=transpose([cross(pos_AJR,u_footR_y); u_footR_y])*fi_footR_y;
    AJX_L(k)=transpose([cross(pos_AJL,u_footL_x); u_footL_x])*fi_footL_x;
    AJX_R(k)=transpose([cross(pos_AJR,u_footR_x); u_footR_x])*fi_footR_x;

    TrBJZ(k)=transpose([cross(pos_LLJ,u_pelvis_z); u_pelvis_z])*fi_pelvis_z;
    TrBJY(k)=transpose([cross(pos_LLJ,u_pelvis_y); u_pelvis_y])*fi_pelvis_y;
    TrBJX(k)=transpose([cross(pos_LLJ,u_pelvis_x); u_pelvis_x])*fi_pelvis_x;
    TpBJZ(k)=fi_pelvis_x(3);
    TpBJY(k)=fi_pelvis_x(2);
    TpBJX(k)=fi_pelvis_x(1);
    
    LNJZ(k)=transpose([cross(pos_LNJ,u_head_z); u_head_z])*fi_head_z;
    LNJY(k)=transpose([cross(pos_LNJ,u_head_y); u_head_y])*fi_head_y;
    LNJX(k)=transpose([cross(pos_LNJ,u_head_x); u_head_x])*fi_head_x;
    
    SJZ_L(k)=transpose([cross(pos_SJL,u_upperArmL_z); u_upperArmL_z])*fi_upperArmL_z;
    SJY_L(k)=transpose([cross(pos_SJL,u_upperArmL_y); u_upperArmL_y])*fi_upperArmL_y;
    SJX_L(k)=transpose([cross(pos_SJL,u_upperArmL_x); u_upperArmL_x])*fi_upperArmL_x;
    
    SJZ_R(k)=transpose([cross(pos_SJR,u_upperArmR_z); u_upperArmR_z])*fi_upperArmR_z;
    SJY_R(k)=transpose([cross(pos_SJR,u_upperArmR_y); u_upperArmR_y])*fi_upperArmR_y;
    SJX_R(k)=transpose([cross(pos_SJR,u_upperArmR_x); u_upperArmR_x])*fi_upperArmR_x;
    
    EJZ_L(k)=transpose([cross(pos_EJL,u_lowerArmL_z); u_lowerArmL_z])*fi_lowerArmL_z;
    EJZ_R(k)=transpose([cross(pos_EJR,u_lowerArmR_z); u_lowerArmR_z])*fi_lowerArmR_z;
    
    ULJZ(k)=transpose([cross(pos_ULJ,u_thorax_z); u_thorax_z])*fi_thorax_z;
    ULJY(k)=transpose([cross(pos_ULJ,u_thorax_y); u_thorax_y])*fi_thorax_y;
    ULJX(k)=transpose([cross(pos_ULJ,u_thorax_x); u_thorax_x])*fi_thorax_x;
    ...
    Freact(:,k)=fi_pelvis_z(1:3);
    Mreact(1,k)=transpose([cross(pos_LLJ,[1;0;0]); [1;0;0]])*fi_pelvis_x;
    Mreact(2,k)=transpose([cross(pos_LLJ,[0;1;0]); [0;1;0]])*fi_pelvis_x;
    Mreact(3,k)=transpose([cross(pos_LLJ,[0;0;1]); [0;0;1]])*fi_pelvis_x;
end
